...
\end{lstlisting}
    \begin{itemize}
      \item \cppcode{[std::ranges::begin(t), std::ranges::end(t))} is a half-open range.
      \item \cppcode{std::ranges::begin(t)} and \cppcode{std::ranges::end(t)} must be valid expressions; their complexity depends on the range type.
      \item If the type of \cppcode{std::ranges::begin(t)} is a forward iterator, then \cppcode{[std::ranges::begin(t), std::ranges::end(t))} is equality preserving (i.e., multi-pass is allowed).
    \end{itemize}
\end{itemize}
\end{frame}
