...
std::list lst{-1, 3, 2, 4, -5, 7};
std::find_if(lst.begin(), lst.end(), [](int x) { return x>0 and x%2==0; });
std::find_if_not(lst.begin(), lst.end(), [](int x) { return x<0; });
\end{lstlisting}
...
std::find_if(lst.begin(), lst.end(), [](int x) { return x>0; });@\pause@

// Lambda with by-value capture. Access to max for reading.
int max = 3;
std::find_if(lst.begin(), lst.end(), [=](int x) { return x>max; });@\pause@

// Lambda with by-reference capture. Access for modification.
int n = 0;
std::find_if(lst.begin(), lst.end(), [&](int x) { n++; return x>max; });
\end{lstlisting}
  ...
  \item \textmark{Return type}: 
    ...
    \end{itemize}
\begin{lstlisting}
std::find(lst.begin(), lst.end(), [](int x) { return x>0; });
std::find(lst.begin(), lst.end(), ([](int x) -> bool { return x>0; }));
\end{lstlisting}
\end{itemize}
\end{frame}
...
\end{lstlisting}
\begin{itemize}
  \item Counts the number of elements that are equal to a value 
        or that satisfy a predicate.
    \begin{itemize}
      \item A unary predicate takes a single parameter and returns a boolean.
    \end{itemize}
