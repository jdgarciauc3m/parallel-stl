...
\item Counting words in a text:
    \begin{itemize}
        ...
        \item A word starts with a non alphanumeric character followed by an alphanumeric character.
        \item If text starts with an alphanumeric character, it starts with a word.
    \end{itemize}
\end{itemize}
...
size_t is_new_word(unsigned char c1, unsigned char c2) {
  bool result = not bool(std::isalpha(c1)) and bool(std::isalpha(c2));
...
\begin{lstlisting}
  size_t count_classic(std::string_view text) {
    ...
    if (std::isalpha(text[0]) != 0) { ++result; }
    return result;
  }
\end{lstlisting}
\end{block}
